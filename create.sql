...
CREATE TABLE CATEGORIE (
   ...
   CONSTRAINT CK_CATEGORIENAAM CHECK (CHAR_LENGTH(RTRIM(CATEGORIENAAM)) >=5)
);
...
CREATE TABLE GEBRUIKER (
   ...
   CONSTRAINT CK_GEBRUIKERSNAAM CHECK (CHAR_LENGTH(RTRIM(GEBRUIKERSNAAM)) >= 5),
   CONSTRAINT CK_VOORNAAM       CHECK (CHAR_LENGTH(RTRIM(VOORNAAM)) >0),
   CONSTRAINT CK_ACHTERNAAM     CHECK (CHAR_LENGTH(RTRIM(ACHTERNAAM)) >= 0),
   CONSTRAINT CK_WACHTWOORD     CHECK (CHAR_LENGTH(RTRIM(WACHTWOORD)) >= 4),
...
CREATE TABLE PRODUCT (
   ...
   CONSTRAINT CK_PRODUCTNAAM    CHECK (CHAR_LENGTH(RTRIM(PRODUCTNAAM)) >= 5),
   CONSTRAINT CK_PRIJS          CHECK (PRIJS > 0),
   CONSTRAINT CK_VOORRAAD       CHECK (VOORRAAD > 0),
   CONSTRAINT CK_OMSCHRIJVING   CHECK(CHAR_LENGTH(RTRIM(OMSCHRIJVING)) > 50),
